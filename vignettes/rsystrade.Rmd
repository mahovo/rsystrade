...
> 
> Work in progress!  
> This document is still very tentative.
...
The intention is to provide as much transparency as possible about how things are calculated. So all functions that implement a formula live in files named `formulas-*.R`. (For `*.R` files containing a large number of functions, Outline in RStudio is your friend.)
...
-   A **position modifier** for each subsystem/instrument.  A position modifier function is analogous to a signal generating function, except that is applied in `update_position_table_row()` and takes a position size in account currency as input and outputs a modified position. An example of a position modifier could be a stop loss function. In addition to position size, the position modifier may take inputs from variables which are available in the env of `update_position_table_row()`, as well as those columns in the instrument data frame which are associated with variable params in a signal generating rule.  
-   Any number of **position multipliers** for each subsystem/instrument. The modified position may be multiplied by any number of *position multiplier functions*.  
-   Any number of **portfolio multipliers**. Portfolio multipliers are applied equally to all position in the system. Portfolio multipliers are useful for things like limiting the over all risk or leverage.  
...
The instrument name is assigned to each data frame as a comment. So we can get the data framename:
```R
comment(inst_data[[i]])
```
...
The name of rule functions should be prefixed by `r_`.

Variations of the same rule are considered separate rules. E.g. *MA(16, 64)* and *MA(20, 80)* would be two separate rules.

A rule function typicially implements a single rule, but the only limitations on a rule are, that   
...
Here are some ideas for multi-rule rule functions:  
  ...
  * One *off-rule*. The *off-rule* is a *signal generating rule* which is applied when no trade is on for the instrument associated with the algo. The *off-rule* generates a trade signal to enter a trade.  
...
Signals are generated by rule functions. In some frameworks, these things are called *forecasts*. I call them signals, because they are *trade signals*, not forecasts. I reserve the term *forecast* for situations where we are making predictions. The idea is to navigate the markets as we observe them, not as we predict them. But this gets semantic, because we *could* generate a signal based on a forecast.

Any *rule function* provided to a system must output a named list, where the first element is the trade signal generated by the rule. This element must be named `signal`. Any number of additional variables may be provided in the list. These will be appended as columns to the signal table of the algo.

The name of *signal generator* functions should be prefixed by `s_`.

The rule variation name is assigned to each signal table data frame as a comment. So we can get the data frame name:
```R
comment(signal_tables[[i]])
```
...
A system contains one position table for each instrument.

**Important**: _Never change the order of the *standard* and *final columns* in the position tables._ Doing so is the road to much pain and suffering. The default columns are the ones generated by `make_system()`. Additional columns produced by position modifiers (and multipliers) in `update_system()` are appended to the right of the default columns. Don't try to rearrange the columns of a system, even if another order may make more sense. Instead, pull out copies of tables (data frames) from the system and do whatever you like with them.
...
Final columns are calculated by `finalize_positions()` when the system is running (the user never needs to call  `finalize_positions()`). If a portfolio multiplier is applied (and not equal to 1), final columns will be recalculated and new columns in position tables will be generated automatically from `portfolio multiplier` output list.
  ...
  * `final_target_pos_units`
  * `final_unbuffered_pos_units`
  * `final_unbuffered_pos_ccy`
  * `buffer_size`
  * `buffer_top`
  * `buffer_bottom`
  * `final_buffered_pos_ccy`
  * `final_pos_change_ccy`
  * `final_target_pos_change_units`
  * `final_pos_change_units`
  * `borrowed_asset_ccy`
  * `percentage_return`
  * `instrument_return`
  * `subsystem_pandl`

### Position modifier and multiplier output columns

Position table columns are automatically generated from outputs of position modifiers and multipliers.
 ...
 * `direction`
 ...
 The following params passed to `update_position_table_row()` from `update_system()` are also valid as inputs to a position modifier function:

 * `inst_data`
 * `position_table` (including rows 1 through `t - 1`)
 * `position_modifier`
...
The user may provide a named list to `make_system()` where the names are instrument names and values are *position modifier functions*. `parse_position_modifiers_list()` creates a named list. Each element corresponds to an instrument, in the order instruments occur in the inst_data list. The name of each element is the name of the instrument. User provided position modifier functions will be assigned to the appropriate element in the list. Any position modifier function for which the name doesn't match any instrument in the system, will be ignored. If no position modifier function is assigned to an instrument, the position of that instrument will not be modified.

Only one *position modifier* may be provided to each instrument.

The first argument of a position modifier function must be `t`.
The second argument of a position modifier function must be `position_size_ccy`.

A modifier function must output a list, and the first element must be the multiplier value, and should be named `modifier_value`.

The instrument name is assigned to each position table data frame as a comment. So we can get the data framename:
```R
comment(position_tables[[i]])
```

The name of *position modifier* functions should be prefixed by `p_`.

## Position multipliers
A position multiplier is a function which is multiplied by the position in account currency, as it is determined by the combined signal. Examples of signal multipliers are

 * risk multipliers.  
 * gates.  

The first argument of a position multiplier function must be `t`.

A multiplier function must output a list, and the first element must be the multiplier value, and should be named `multiplier_value`.

One or more position multipliers are applied to a (modified) position size by multiplication. The output of each multiplier is appended to the positions table through the additional elements in the output list of the position multiplier.

> **Note**  
> 
> Obvously one should be careful about applying multiple position multipliers other than gates. The combined effect of multiple gates is still a gate. But for instance, if a risk multiplier is limiting the instrument risk to a minimum value, appying a second multiplier may also modify the risk.

While only one *position modifier* may be provided to each instrument, a list of *position multipliers* may be provided.

The name of *position multiplier* functions should be prefixed by `m_`.

**Important**: In the position tables for each instrument, columns associated with modifiers and multipliers are appended to the right, eventhough modifiers and multipliers are calculated before some of the default columns. The final position in the currency of the account is found in the `final_pos_size_ccy` column!

 * `position_size_ccy` is the position size in the currency of the account as calculated from the combined signal *before* position modification.  
 * `modifier_value` is the modified position size in the currency of the account. This is calculated by applying a position modifier function to the `position_size_ccy`.  
 * `multiplier_value` is the output value of a multiplier function.  
 * `combined_multiplier` is the product of all multipliers for an instrument.  
 * `modified_position_size_ccy` is the position size calculated by `modify_position()`, i.e. by applying the *position modifier* function and the *position multiplier* function(s) to `position_size_ccy`. In other words, `modified_position_size_ccy` is equal to `modifier_value` multiplied by `combined_multiplier`.  

If a variable param in `position_multiplier` exists as a column in the instrument data, the `position multiplier` gets the data from that column. Additional valid params generated by `update_position_table_row()` are:  
...
The following params passed to `update_position_table_row()` from `update_system()` are also valid as inputs to a position multiplier function:
...
### Gates
A gate function is a function which outputs either 0 or 1. E.g. if we want to implement a *stop loss* position modifier, and we want to control when a new position may be entered after the stop loss was invoked, we can do this with a gate. (See the built-in function `m_block_same_direction_entry()`). In this case the *position modifier function* should first calculate the modified position based on the stop loss rule, and then determine if the conditions for opening a new position are met. If the conditions are met, the gate should return 1, otherwise 0. If a stop loss was invoked on a long position at `t - 1`, we may get a new long signal at `t`. If the gate determines, that a new long position at `t` is not allowed, the position will be multiplied by 0, returning a modified position of 0. An alternative way of doing this could be to implement a gate as part of a signal generating function. However, if multiple rules are applied to the same instrument, there would be redundancy due to the gate calculations being performed for each rule.


# Portfolios
## Portfolio multipliers

See also `help(combine_portfolio_multipliers)`.

Portfolio multipliers are calculated when all positions have been calculated for a given time index `t`. If the value of the multiplier is not 1, the final position columns of each position table will be recalculated.

Portfolio multipliers are similar to position multipliers, just on the portfolio level. 

Portfolio multipliers are specified as a parameter to `make_system()`. Individual portfolio multiplier functions are referenced as function names refering to either built-in multiplier functions (defined in `portfolio_multipliers.R`) or loaded into the global environment at run time.
...
The first input to a portfolio multiplier function must be `t`.

The first output to a portfolio multiplier must be the multiplier value, which must be a single value in $[0, 1]$.

Portfolio multiplier functions are combined according to the method specified in `my_system$config$portfolio_multiplier$combi_method`. The default is to use the minimum value. Valid combination methods are `min`, `max`, `add` and `multiply`.

> **Note**  
> 
> Obvously one should be careful about applying multiple portfolio multipliers other than gates. The combined effect of multiple gates is still a gate. But for instance, if a risk multiplier is limiting the portfolio risk to a maximum value, appying a second multiplier, which limits the leverage, will also modify the risk. An appropriate approach may be to take the minimum of a number of multipliers, so that only the smallest multiplier is applied. This could be implemented by combining several multipliers in a single multiplier function.

Unlike *position multipliers* and *position modifiers*, variable params for *portfolio multipliers* don't take input directly from `inst_data`. Instrument data can be accessed via the system variables, which are available to portfolio multiplier functions.
...
The name of *portfolio multiplier* functions should be prefixed by `o_`.
...
# List of prefixes in *rsystrade*

* `r_` is the prefix for *rule* function names.  
* `s_` is the prefix for *signal generator* function names.  
* `p_` is the prefix for  *position modifier* function names.  
* `m_` is the prefix for *position multiplier* function names.  
* `o_` is the prefix for  *portfolio multiplier* function names.  
