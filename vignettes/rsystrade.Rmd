...
The rule variation name is assigned to each signal table data frame as a comment. So we can get the data framename:
```R
...
> **Note**  
> 
> Obvously one should be careful about applying multiple position multipliers other than gates. The combined effect of multiple gates is still a gate. But for instance, if a risk multiplier is limiting the portfolio risk to a maximum value, appying a second multiplier, which limits the leverage, will also modify the risk.
