...
A multiplier function must output a list, and the first element must be the multiplier value, and should (by convention) be named `multiplier_value`.
...
**Important**: In the position tables for each instrument, columns associated with modifiers and multipliers are appended to the right. This means that the `modified_position_size_ccy` and `final_position_size_ccy` columns may be separated from the `position_size_ccy` by several columns. The final position in the currency of the account is found in the `final_position_size_ccy` column!
 ...
 * `modified_position_size_ccy` is the modified position size in the currency of the account. This is calculated by applying a position modifier function to the `position_size_ccy`.  
 * `final_position_size_ccy` is final position in the currency of the account. This is calculated by multiplying the `modified_position_size_ccy` by one or more position multiplier functions. Position multiplier functions are applied inside `modify_position()`.
