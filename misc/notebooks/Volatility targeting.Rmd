...
Here, the mean of $[\tilde{X}]$ is not zero, so we shift $[\tilde{X}]$ to get a zero mean.   
...
## Generate X_tilde num_test times.
...
## For each test, get the standard deviation of the X_tilde signal, i.e. the combined signal.
norm_test <- data.frame(unlist(lapply(norm_test_data, sd)))
names(norm_test) <- "sd"

## For each test, get the MAV of the X_tilde signal, i.e. the combined signal.
...
- Ifthe contributions from $\mathbf{D}$ and $K$ cancel each other out, we have:  
...
- If we use $w^THw$ instead of $w^T\Sigma w$, the diversification multiplier can be many (millions of) orders of magnitude bigger than the theoretical MAV that we are targeting, of $\sigma$ is small and $\mu$ is not $0$.
...
- For the heavily skewed signals, the picture is quite different than for normal distributed data.  
