...
Large data frame where first row is given.
For each row, copy element above and add log of the element to the left.
No vectorized functions.

This roughly represents the process of performing different stages for each row.
...
+ Version 1:
  ...
  + Inner functions take no input.
    + Modify single variables in env of outer function based on 
    elements in `df` (also in env of `version1()`.
    + Each inner function is called in turn inside `version1()`.
+ Version 2:
  + Function `version2()` takes `df` as input.
  + Function `version2()` calls external functions which take `df` and variables
  from env of `version2()`
  + Each external function takes an element from `df` in their own env and 
  outputs a modified value.
  + Each external function is called in turn inside `version2()` such that one 
  modifies the output of the previous one.
  + Finally a vector with all the individual variables replaces a row in `df`.
+ Version 3:
...
between the FP and OOP approaches.

Version 6 seems to show, that it's fine to store all variables in a large data frame. However, this turns not to be true, as seen in `Profiling 2.Rm` and `Profiling 3.Rmd`.

Question: Can we trust, that there is no difference between the approaches in
version 1 - 5? Test this...
...
## First element must be 2's 
## (If df[1, 1] <- 1, log(row_n[1] will be 0, and nothing will change.)
